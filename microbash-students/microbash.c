...
void free_command(command_t * const c)
{
	...
	free(c->in_pathname);
	free(c->out_pathname);
	//free(c); // this is not needed, as the caller will free the memory
	/*** TO BE DONE END ***/
}
...
void free_line(line_t * const l)
{
	...
	free(l->commands);
	l->n_commands=0;
	//free(l); // this is not needed, as the caller will free the memory
	/*** TO BE DONE END ***/
}
...
check_t check_redirections(const line_t * const l)
{
	...
	/*** TO BE DONE START ***/
	for(int i=0; i<l->n_commands; i++){
		if(i==0 && l->commands[i]->in_pathname)
			return CHECK_FAILED;
		if(i==l->n_commands-1 && l->commands[i]->out_pathname)
			return CHECK_FAILED;
	}
	/*** TO BE DONE END ***/
	return CHECK_OK;
...
check_t check_cd(const line_t * const l)
{
	 ...
	 */
	/*** TO BE DONE START ***/
	for(int i=0; i<l->commands[0]->n_args; i++)
		if(strcmp(l->commands[0]->args[i], CD)==0)
			if(l->n_commands!=1 || l->commands[0]->in_pathname || l->commands[0]->out_pathname || l->commands[0]->n_args!=2)
				return CHECK_FAILED;
	/*** TO BE DONE END ***/
	return CHECK_OK;
}
	...
	while((pid=wait(&status))!=-1){
		if(WIFEXITED(status)){
			if(WEXITSTATUS(status)!=0)
				fprintf(stderr, "FAIL: Child %d exited with status %d\n", pid, WEXITSTATUS(status));
		}
		else if(WIFSIGNALED(status)){
			fprintf(stderr, "FAIL: Child %d killed by signal %d (%s)\n", pid, WTERMSIG(status), strsignal(WTERMSIG(status)));
		}
	}
	/*** TO BE DONE END ***/
}
...
void run_child(const command_t * const c, int c_stdin, int c_stdout)
{
	...
	pid_t pid = fork();
	if(pid==-1){
		perror("FAIL to: Create a child process");
	}
	if(pid==0){
		redirect(c_stdin, STDIN_FILENO);
		redirect(c_stdout, STDOUT_FILENO);
		if(execvp(c->args[0], c->args)==-1){
			perror("FAIL to: Execute the command specified in c->args[0] with the corresponding arguments c->args");
			exit(EXIT_FAILURE);
		}
	}
	/*** TO BE DONE END ***/
...
void change_current_directory(char *newdir)
{
	...
	/*** TO BE DONE START ***/
	if(chdir(newdir)==-1){
		perror("FAIL to: Change the current working directory to newdir");
	}
	/*** TO BE DONE END ***/
}
...
void execute_line(const line_t * const l)
{
	...
	for(int a=0; a<l->n_commands; ++a) {
		...
		if (c->in_pathname) {
			...
			/*** TO BE DONE START ***/
			curr_stdin = open(c->in_pathname,0);
			if(curr_stdin==-1){
				perror("FAIL to: Open c->in_pathname and assign the file-descriptor to curr_stdin");
			}
			/*** TO BE DONE END ***/
		}
		if (c->out_pathname) {
			 ...
			 * (handling error cases) */
			/*** TO BE DONE START ***/
			curr_stdout = open(c->out_pathname,0);
			if(curr_stdout==-1){
				perror("FAIL to: Open c->out_pathname and assign the file-descriptor to curr_stdout");
			}
			/*** TO BE DONE END ***/
			...
			/*** TO BE DONE START ***/
			if(pipe(fds)==-1){
				perror("FAIL to: Create a pipe in fds, and set FD_CLOEXEC in both file-descriptor flags");
			}
...
int main()
{
	...
	for(;;) {
		...
		/*** TO BE DONE START ***/
		pwd = getcwd(NULL, 0);
		my_malloc(strlen(pwd) + prompt_suffix_len + 1);
		/*** TO BE DONE END ***/
